...
describe('event-source ids', () => {
    it('round-trips a source/item pair', () => {
        const id = sourceEventId('boards-due', 'r8f3k2m9x1p7q4w')
        expect(id).toBe('src:boards-due:r8f3k2m9x1p7q4w')
    ...
    it('keeps colons inside the item id intact', () => {
        ...
        const parsed = parseSourceEventId(sourceEventId('boards-due', 'a:b:c'))
        expect(parsed).toEqual({ sourceId: 'boards-due', itemId: 'a:b:c' })
    })
    ...
    it('rejects non-source and malformed ids', () => {
        ...
        expect(parseSourceEventId('src:boards-due:')).toBeNull()
        expect(parseSourceEventId('src::item')).toBeNull()
    })

    it('derives the pseudo-calendar id from the source id alone', () => {
        expect(sourceCalendarId('boards-due')).toBe('src:boards-due')
    })
})
